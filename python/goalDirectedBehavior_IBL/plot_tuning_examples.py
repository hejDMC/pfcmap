...
param_dict = {}
param_dict['choice'] = {'var': 'choice', 'align_type': 'firstMovement_times', 'vals': [-1, 1],
                        'analysis_win': [-0.1, 0.], \
                        'fix_dict': {'stimdir': 'stimdirection'}, 'tbounds_show': [-0.4, 0.3],
                        'deltat_range': [0.08, 2.]}
param_dict['stimulus'] = {'var': 'stimdirection', 'align_type': 'start_time', 'vals': [-1, 1],
                          'analysis_win': [0., 0.1], \
                          'fix_dict': {'choice': 'choice'}, 'tbounds_show': [-0.3, 0.4]}
param_dict['feedback'] = {'var': 'feedbackType', 'align_type': 'feedback_times', 'vals': [-1, 1],
                          'analysis_win': [0, 0.2], \
                          'fix_dict': {'choice': 'choice'}, 'tbounds_show': [-0.3,
                                                                             0.4]}  # cannot do both choice and stim here as fixed as then feedback is a natural given...
...
for signiftag in ['signif','nosignif']:
    for vartag in ['choice','stimulus','feedback']:

        exfiles = glob(os.path.join(figdir_mother,vartag,signiftag,'*.png'))#example files to be recreated in different format
        myrecids,myuids0 = np.vstack([os.path.basename(fname).split('__')[:2] for fname in exfiles]).T
        ruids = [recid+'__'+uid for recid,uid in zip(myrecids,myuids0)]
        urecids = np.unique(myrecids)
        metricsfiles = [os.path.join(srcdir,'metrics_%s.h5'%recid) for recid in urecids]
        ...
        unit_filter = lambda uobj: str(uobj.id) in ruids #S.check_layers(uobj.layer,layers_allowed)

        Usel = [U for U in Units if unit_filter(U)]
        #Us = [U for U in Units if U.id=='CSH_ZAD_024_2020-06-08-probe01__1182']
        ...
        for recid in urecids:
            ...
            plot_info_collector[recid]['vardata'][vartag] = {'cond_deltat': cond_deltat, 'align_times': align_times,
                                                             'conds_collected': conditions_collected}

        vartags = [vartag]

        cccp_rec_dict, dimdict = iblfns.load_cccp_data(urecids, dstdir, myrun)
        iblfns.assing_tunings(Usel, cccp_rec_dict, dimdict, get_significance=True, pthr=pthr, pthr_naive=pthr_naive,
                              cccpthr=cccpthr)
        if vartag == 'choice':
