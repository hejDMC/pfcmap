...
srcdir = 'D:/Carlen/Pete_Rudebeck/NWB'
tintfile_dir = 'D:/Carlen/Intermediate/preprocessing/metrics_extraction/timeselections/timeselections_Rudebeck'
dstpath = 'D:/Carlen/Intermediate/preprocessing/metrics_extraction/quantities_all_meanvar/Pete_Rudebeck'
...
with Progress() as progress:
    task = progress.add_task("Extraction of trial start times. Current file : None", total = nfiles)
    for ff,filename in enumerate(filepool):
        aRec = dc.RecPassive(filename,cfgpath,dspath)
        progress.update(task, description='EXTRACTING %s %i'%(aRec.id,ff+1), advance=1)

        for tintdur in np.arange(1,4):
            tintfile = glob(os.path.join(tintfile_dir,'%s*prestim%i*.h5'%(aRec.id,tintdur)))
