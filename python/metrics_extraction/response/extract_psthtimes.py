...
ttag = 'psth'
block_licking = True
savepattern = '%s__TSEL%s{}to{}__STATE%s%s.h5'.format(int(precut),int(postcut*10)) #% (aRec.id, ttag, statetag,reftag)
...
dstdir = 'ZENODOPATH/preprocessing/metrics_extraction/timeselections/timeselections_psth'#
...
stimdict = {'Passive': {'stimnames': ['passive_10kHz','passive_5kHz','passive_bluenoise'],'stimamps':[3.,2.,1.]},
 ...
 'Attention':{'stimnames': ['AudStim_first']}}
...
for recid,quietdet_src in zip(recids,quietdet_srcs):
    ...
    stimtimes = np.hstack([aRec.h_info['stimulus/presentation/%s/timestamps' % stimname][()] for stimname in stimnames])
    amp_inds = np.arange(len(stimtimes))
    if 'stimamps' in stimdict[exptype].keys():
        amps_allowed = stimdict[exptype]['stimamps']
        stimamps = np.hstack([aRec.h_info['stimulus/presentation/%s/data' % stimname][()] for stimname in stimnames])
        amp_inds = np.array([ii for ii,amp in enumerate(stimamps) if amp in amps_allowed])
        print('amp-selection: using %i out of %i stimuli'%(len(amp_inds),len(stimtimes)))

    stimtimes = np.sort(stimtimes[amp_inds])
