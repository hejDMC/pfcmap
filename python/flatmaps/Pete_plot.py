...
'''
pathpath = 'PATHS/filepaths_Pete.yml'
myrun = 'runPete3'
...
region_file = os.path.join(pathdict['tessellation_dir'],'flatmap_regions.h5')
...
figdir_mother = os.path.join(pathdict['figdir_root'],'SOMruns',myrun,'%s__kShape%i_%i/clustering/Nc%s/%s/enrichments/%s'%(myrun,kshape[0],kshape[1],ncluststr,cmethod,roi_tag))
...
with h5py.File(statsfile,'r') as hand:
    ...
    # Use region-level labels (drop ROI subparts after '|') for plotting
    sdict['alabels'] = np.array([replace_fn(aval.split('|')[0]) for aval in sdict['avals1']])
...
unique_roiruns = ['nolayRois']#



with h5py.File(region_file,'r') as hand:
    polygon_dict= {key: hand[key][()] for key in hand.keys()}
    #get the fill attribute for each region if it exists, otherwise default to 'none'
    region_colors = {key: hand[key].attrs.get('fill', 'none') for key in hand.keys()}

# Compute bounds for proper scaling
all_verts = np.concatenate([verts * np.array([1, -1]) for verts in polygon_dict.values()])
x_min, y_min = all_verts.min(axis=0)
x_max, y_max = all_verts.max(axis=0)

# Redefine set_mylim to use computed bounds
def set_mylim(myax):
    myax.set_xlim([x_min, x_max])
    myax.set_ylim([y_min, y_max])
...
f,ax = plt.subplots()
ttools.colorfill_polygons(ax, polygon_dict, region_colors, na_col='k', ec='k',
                           show_cmap=False, mylimfn=set_mylim)
figsaver(f,'regions_colored')
...
for basetag in ['levels','emat_zerod','emat']:
    ...
    for thistag in seltags:
        ...
        # Map region labels to colors using the fill attribute from the HDF5 file
        for region in srcdict['alabels']:
            if region in region_colors:
                region_colors[region] = region_colors[region]
            else:
                for rc in region_colors.keys():
                    if region.startswith(rc):
                        region_colors[region] = region_colors[rc]
                        break
                region_colors[region] = 'none'  # default color for regions without a specified fill attribute
        cdict_avals = {region: region_colors[region] for region in srcdict['alabels']}
        ...
        if thistag in unique_roiruns:
            ...
            # Use region-level labels for coloring the flatmap
            plotdict_z = {region: zmat[rr] for rr, region in enumerate(sdict['alabels'])}
            ...
            for cc, clust in enumerate(np.arange(ncl)):
                ...
                ttools.colorfill_polygons(ax, polygon_dict, plotdict_z, subkey=cc, cmap=cmap_z, clab='', na_col='grey',nancol=nancol, ec='k',
            ...
            if not plot_just_enrichment:
                ...
                for nn, nclusters in enumerate(NCvec):
                    ...
                    plotdict = {region:mylab for region,mylab in zip(srcdict['alabels'],mylabs)}
                    labdict[nclusters] = {key:[plotval,mymap.to_rgba(plotval)] for key,plotval in plotdict.items()}

                    ttools.colorfill_polygons(ax, polygon_dict, plotdict, cmap=mymap, clab='', na_col='grey',nancol=nancol, ec='k',
                                            show_cmap=False, mylimfn=set_mylim)
                f.suptitle(titlestr)
